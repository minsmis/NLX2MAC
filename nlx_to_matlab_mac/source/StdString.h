...
public:
	...
	CT* GetBuf(int nMinLen=-1)
	{
		if ( static_cast<int>(this->size()) < nMinLen )
			CStdStr<char>::resize(static_cast<MYSIZE>(nMinLen));
	...
	{
		MYSIZE nIdx	= CStdStr<char>::find(szSub);
		return static_cast<int>(MYBASE::npos == nIdx ? -1 : nIdx);
	}
	...
	{
		MYSIZE nIdx = CStdStr<char>::find_first_of(szCharSet);
		return static_cast<int>(MYBASE::npos == nIdx ? -1 : nIdx);
	}
	...
	{
		return CStdStr<char>::substr(0, static_cast<MYSIZE>(nCount));
	}
	...
	{
		return CStdStr<char>::substr(static_cast<MYSIZE>(nFirst), static_cast<MYSIZE>(nCount));
	}
	...
	int Remove(CT ch)
	{
		MYSIZE nIdx		= 0;
		int nRemoved	= 0;
		while ( (nIdx=CStdStr<char>::find_first_of(ch)) != MYBASE::npos )
		{
			CStdStr<char>::erase(nIdx, 1);
			nRemoved++;
		}
		return nRemoved;
	}
	...
	MYTYPE Right(int nCount) const
	{
		...
		return CStdStr<char>::substr(this->size()-static_cast<MYSIZE>(nCount));
	}
