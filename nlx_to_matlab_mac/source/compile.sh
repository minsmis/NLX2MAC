...
#
# the first few lines are parameters that need to be adjusted (see README_FROM_AUTHORS_FROM_AUTHORS file).
#
...
PLATFORM="MMAC"     #should be 32PC, 64PC, 32MAC or 64MAC, or MMAC (APPLE SILICON)
...
64MAC)
    ...
    ;;
MMAC)
    echo "Compiling the M1/M2 MAC OS version"
    cp $SRCDIR/compatibility64.h $SRCDIR/compatibility.h
    CFLAGS="$CFLAGS_COMMON -arch arm64"
    ;;
*)
